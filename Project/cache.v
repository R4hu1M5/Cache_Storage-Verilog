...
module cache(instruction, clk, signal);
    ...
    always@(posedge clk)
    begin: loop_block
       if(cache_memory[index][20]) // if valid bit is set i.e. data is present
       begin
        if(tag == cache_memory[index][19:0])
        begin
        ...
        begin
          cache_memory[index][19:0] <= tag; // When the tags are not the same, we overwrite the previous data with new data.
          signal <= 0;
          disable loop_block;
        end
      end
      ...
      begin
        ...
        cache_memory[index][19:0] <= tag;
        cache_memory[index][20] <= 1; // storing new data at that index and setting valid bit
        disable loop_block;
      end
    end
endmodule
